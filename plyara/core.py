...
class Parser:
    ...
    def __init__(
        self, console_logging=False, store_raw_sections=True, meta_as_kv=False
    ):
    ...
    def _add_element(self, element_type, element_value):
        ...
        if element_type == ElementTypes.RULE_NAME:
            ...
            if self.store_raw_sections:
                if self._meta_start:
                    self.current_rule["raw_meta"] = self._raw_input[
                        self._meta_start : self._meta_end
                    ]

                if self._strings_start:
                    self.current_rule["raw_strings"] = self._raw_input[
                        self._strings_start : self._strings_end
                    ]

                if self._condition_start:
                    self.current_rule["raw_condition"] = self._raw_input[
                        self._condition_start : self._condition_end
                    ]
...
class Plyara(Parser):
    ...
    def t_STRING_error(t):
        ...
        raise ParseTypeError(
            "Illegal string character: {!r}, at line: {}".format(
        ...
        if lower_bound and upper_bound:
            if not 0 <= int(lower_bound) <= int(upper_bound):
                raise ParseValueError(
                    "Illegal bytestring jump bounds: {}, at line: {}".format(
    ...
    def t_BYTESTRING_end(t):
        ...
        if t.lexer.bytestring_group != 0:
            raise ParseValueError(
                "Unbalanced group in bytestring: {}, at line: {}".format(
                    t.value, t.lexer.lineno
                ),
                t.lexer.lineno,
                t.lexer.lexpos,
    ...
    def t_BYTESTRING_error(t):
        ...
        """
        raise ParseTypeError(
            "Illegal bytestring character : {}, at line: {}".format(
                t.value[0], t.lexer.lineno
            ),
            t.lexer.lineno,
            t.lexer.lexpos,
    ...
    def t_REXSTRING_error(t):
        ...
        """
        raise ParseTypeError(
            "Illegal rexstring character : {!r}, at line: {}".format(
                t.value[0], t.lexer.lineno
            ),
            t.lexer.lineno,
            t.lexer.lexpos,
    ...
    def t_error(t):
        ...
        """
        raise ParseTypeError(
            "Illegal character {!r} at line {}".format(t.value[0], t.lexer.lineno),
            t.lexer.lineno,
            t.lexer.lexpos,
        )
    ...
    def p_meta_kv(self, p):
        ...
        self._add_element(
            ElementTypes.METADATA_KEY_VALUE,
            (
                key,
                value,
            ),
    ...
    def _parse_string_kv(self, p, string_type):
        ...
        self._add_element(
            ElementTypes.STRINGS_KEY_VALUE,
            (
                key,
                value,
                string_type,
    ...
    def p_xor_mod_args(self, p):
        ...
        for i, x in enumerate(mods):
            ...
            if 0 <= mod_int <= 255:
                ...
                message = (
                    "String modification value {} not between 0-255 on line {}".format(
                        x, p.lineno(1 + i)
                    )
                )
                raise ParseTypeError(message, p.lineno, p.lexpos)
        if mod_int_list[0] > mod_int_list[-1]:
            ...
            message = (
                "String modification lower bound exceeds upper bound on line {}".format(
                    line_no
                )
            )
            raise ParseTypeError(message, p.lineno, p.lexpos)
    ...
    def p_base64_with_args(self, p):
        ...
        if len(b64_data) != 64:
            raise Exception(
                "Base64 dictionary length {}, must be 64 characters".format(
                    len(b64_data)
                )
            )
    ...
    def p_condition(self, p):
        ...
        if p[1] == "$":
            message = "Potential wrong use of anonymous string on line {}".format(
                p.lineno(1)
            )
            logger.info(message)
    ...
    def p_error(self, p):
        ...
        if not p:
            ...
            message = "Unknown text {} for token of type {} on line {}".format(
    ...
    def _process_string_with_escapes(t, escape_chars=None):
        ...
        if t.lexer.escape == 1 and t.value in escape_chars or t.value == "\\":
            ...
            if t.value.lower() in string.hexdigits:
                ...
                raise ParseTypeError(
                    "Invalid hex character: {!r}, at line: {}".format(
                        t.value, t.lexer.lineno
                    ),
                    t.lexer.lineno,
                    t.lexer.lexpos,
            ...
            raise ParseTypeError(
                "Invalid escape sequence: \\{}, at line: {}".format(
                    t.value, t.lexer.lineno
                ),
                t.lexer.lineno,
                t.lexer.lexpos,
    ...
    def _add_string_modifier(self, p):
        ...
        if mod_str in self.string_modifiers:
            message = "Duplicate string modifier {} on line {}".format(
                mod_str, p.lineno(1)
            )
            raise ParseTypeError(message, p.lineno, p.lexpos)
        if mod_str in self.EXCLUSIVE_TEXT_MODIFIERS:
            ...
            if excluded_modifiers:
                ...
                message = "Mutually exclusive string modifier use of {} on line {} after {} usage".format(
                    mod_str, p.lineno(1), prev_mod_str
                )
                raise ParseTypeError(message, p.lineno, p.lexpos)
        if self.string_modifiers:
            # Convert previously created modifiers with args to strings
            if mod_str.startswith("base64") and isinstance(
                self.string_modifiers[-1], YaraBase64
            ):
...
class YaraBase64(str):
    ...
    def __repr__(self):
        ...
        if self.modifier_alphabet is None:
            ...
            return "{}({})".format(
                self.__class__.__name__, repr(self.modifier_alphabet)
            )
